...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Widget build(BuildContext context) {
    return ScreenFrame(title: 'Settings', subtitle: 'Configuration backend, auth, usage et facturation.', trailing: IconButton.filled(onPressed: () => setState(_load), icon: const Icon(Icons.refresh_rounded)), child: FutureBuilder<({List<UsageMetric> usage, BillingPlan plan})>(future: _future, builder: (context, snapshot) {
      ...
      return Column(children: [
        PremiumCard(child: Row(children: [CircleAvatar(radius: 28, backgroundColor: AppTheme.accent.withValues(alpha: 0.22), child: const Text('HZ', style: TextStyle(fontWeight: FontWeight.w900))), const SizedBox(width: 14), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Hz Technology', style: TextStyle(fontWeight: FontWeight.w900)), SizedBox(height: 4), Text('Session authentifiée', style: TextStyle(color: AppTheme.muted))])), const Icon(Icons.verified_rounded, color: AppTheme.accent2)])),
        const SizedBox(height: 16),
        PremiumCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Backend configuration', style: TextStyle(fontWeight: FontWeight.w900)), const SizedBox(height: 8), Text(AppConfig.useApi ? AppConfig.apiBaseUrl : 'Fallback mock local actif', style: const TextStyle(color: AppTheme.muted, height: 1.45)), const SizedBox(height: 10), Text('Login ${AppConfig.authLoginEndpoint} · Register ${AppConfig.authRegisterEndpoint}', style: const TextStyle(color: AppTheme.muted, fontSize: 12)), const SizedBox(height: 14), SwitchListTile.adaptive(value: AppConfig.useApi, onChanged: null, title: const Text('API réelle activée'), contentPadding: EdgeInsets.zero)])),
        const SizedBox(height: 16),
